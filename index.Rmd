...
##chart 1
```{r line_2016, echo = FALSE, message =  FALSE}
```
```{r line_2020, echo = FALSE, message =  FALSE}
```  
For the first chart, we decided to create two separate line charts to showcase 
the number of votes per state in the 2016 presidential election, `total_votes16`, versus the number of votes per states in the 2020 presidential election, `total_votes20`. By
doing this, we are able to observe which states had higher voter turnout, had lower
voter turnout, as well as the difference between the two different elections. In 
addition, with this information, we can apply it to what we know about voter 
disparities and analyze if there are any correlation between that information and 
the evident numeric data. 
...
Observing this information allowed us to see which racial groups dominates holds 
a majority position in which state, and whether or not this demographic trends
holds any affects for voter turnout and preference of a certain candidate.
...
2020 election. By analyzing this, we wanted to see the trends and difference between 
the two major options for the specified swing states, as well as the amount of votes
that ultimately lead to the winner of that state's electoral votes. 
